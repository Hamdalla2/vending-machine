import "./candy.css"
import { Component } from 'react';

class Candy extends Component{
render() {
  return (
    <>
    {/* first row */}
    <img className="candy-item" id="11" style={{left:"27vw",top:"0vh"}} src="https://cdn.shopify.com/s/files/1/0061/0782/4216/products/C5C52833-8F38-4E9A-AB72-F344A09F66E6_600x.png?v=1613438867" alt="candy item"/>
    <img className="candy-item" id="12" style={{left:"34vw",top:"0vh"}} src="https://www.thecandyjarby1892.com/wp-content/uploads/2020/03/chupete.png" alt="candy item"/>
    <img className="candy-item" id="13" style={{left:"40vw",top:"0vh"}} src="https://i.imgur.com/4OySEcp.png" alt="candy item"/>
    <img className="candy-item" id="14" style={{left:"46vw",top:"0vh"}} src="https://upload.wikimedia.org/wikipedia/commons/9/90/Candy-Cane-Classic_thumbnail.png" alt="candy item"/>    
    <img className="candy-item" id="15" style={{left:"50vw",top:"0vh"}} src="https://www.yogurt-land.com/assets/634.png" alt="candy item"/>
    <img className="candy-item" id="16" style={{left:"56vw",top:"0vh"}} src="https://cdn.shopify.com/s/files/1/0511/5969/5552/files/bliss-home-header-cone_2400x.png?v=1614366445" alt="candy item"/>
    <img className="candy-item" id="17" style={{left:"61vw",top:"0vh"}} src="https://www.haagen-dazs.ca/sites/default/files/hdz-prod-bars_classics_va_mini.png" alt="candy item"/>
    <img className="candy-item" id="18" style={{left:"67vw",top:"0vh"}} src="https://www.mcdonalds.co.za/media/products/cone/Cone.png" alt="candy item"/>
    <img className="candy-item" id="19" style={{left:"72vw",top:"0vh"}} src="https://www.oreoicecream.com/imageGen.ashx?image=/images/products/GBR/20042012245266Oreo110mlConoclassic31011093front.png&width=600&Height=600&constrain=true" alt="candy item"/>    

    <label className="candy-label" style={{left:"28vw",top:"10vh"}}>11</label> 
    <label className="candy-label" style={{left:"35vw",top:"10vh"}}>12</label> 
    <label className="candy-label" style={{left:"41vw",top:"10vh"}}>13</label> 
    <label className="candy-label" style={{left:"46vw",top:"10vh"}}>14</label> 
    <label className="candy-label" style={{left:"51vw",top:"10vh"}}>15</label> 
    <label className="candy-label" style={{left:"56vw",top:"10vh"}}>16</label> 
    <label className="candy-label" style={{left:"62vw",top:"10vh"}}>17</label> 
    <label className="candy-label" style={{left:"68vw",top:"10vh"}}>18</label> 
    <label className="candy-label" style={{left:"73vw",top:"10vh"}}>19</label> 
    {/* second row */}
    <img className="candy-item" id="21" style={{left:"27vw",top:"15vh"}} src="http://static1.squarespace.com/static/5ca3b2217980b3338106d9bf/5e9d799f49f74c43e7d4d9e2/5e8c709317a9377ba007b6af/1592901778463/Coca+Cola+300ml+Can.png?format=1500w" alt="candy item"/>
    <img className="candy-item" id="22" style={{left:"34vw",top:"15vh"}} src="https://www.mobicool.com/ui/productimages/98/13/dometic_cool-can-10_9600003942_69813_11.png?mw=580&mh=580&hash=274C18E840DFD8E457D2387BA6F840B0" alt="candy item"/>
    <img className="candy-item" id="23" style={{left:"40vw",top:"15vh"}} src="https://terroirs.co/wp-content/uploads/2016/09/p_7_7_5_775-Sprite-can-33cL.jpg" alt="candy item"/>
    <img className="candy-item" id="24" style={{left:"45vw",top:"15vh"}} src="https://www.jerseymikes.com/media/static/bedrock/lg/bottled-drink.png" alt="candy item"/>    
    <img className="candy-item" id="25" style={{left:"50vw",top:"15vh"}} src="https://pizzahousealmere.nl/wp-content/uploads/2020/02/Fanta-orange-canada.png" alt="candy item"/>
    <img className="candy-item" id="26" style={{left:"56vw",top:"15vh"}} src="https://www.tingstad.com/fixed/images/Main/1550834217/36195.png" alt="candy item"/>
    <img className="candy-item" id="27" style={{left:"61vw",top:"15vh"}} src="https://www.laudinella.ch/delivery-shop/wp-content/uploads/2014/12/RedBull.png" alt="candy item"/>
    <img className="candy-item" id="28" style={{left:"67vw",top:"15vh"}} src="https://repracores.pt/wp-content/uploads/2019/10/pepsi-cola.png" alt="candy item"/>
    <img className="candy-item" id="29" style={{left:"72vw",top:"15vh"}} src="https://i.imgur.com/MOHkCtP.png" alt="candy item"/>    
    
    <label className="candy-label" style={{left:"28vw",top:"25vh"}}>21</label> 
    <label className="candy-label" style={{left:"35vw",top:"25vh"}}>22</label> 
    <label className="candy-label" style={{left:"41vw",top:"25vh"}}>23</label> 
    <label className="candy-label" style={{left:"46vw",top:"25vh"}}>24</label> 
    <label className="candy-label" style={{left:"51vw",top:"25vh"}}>25</label> 
    <label className="candy-label" style={{left:"57vw",top:"25vh"}}>26</label> 
    <label className="candy-label" style={{left:"62vw",top:"25vh"}}>27</label> 
    <label className="candy-label" style={{left:"68vw",top:"25vh"}}>28</label> 
    <label className="candy-label" style={{left:"73vw",top:"25vh"}}>29</label>
    {/* third row */}
    <img className="candy-item" id="31" style={{left:"27vw",top:"30vh"}} src="https://cdn.shopify.com/s/files/1/0061/0782/4216/products/C5C52833-8F38-4E9A-AB72-F344A09F66E6_600x.png?v=1613438867" alt="candy item"/>
    <img className="candy-item" id="32" style={{left:"34vw",top:"30vh"}} src="https://www.thecandyjarby1892.com/wp-content/uploads/2020/03/chupete.png" alt="candy item"/>
    <img className="candy-item" id="33" style={{left:"40vw",top:"30vh"}} src="https://i.imgur.com/4OySEcp.png" alt="candy item"/>
    <img className="candy-item" id="34" style={{left:"46vw",top:"30vh"}} src="https://upload.wikimedia.org/wikipedia/commons/9/90/Candy-Cane-Classic_thumbnail.png" alt="candy item"/>    
    <img className="candy-item" id="35" style={{left:"50vw",top:"30vh"}} src="https://www.yogurt-land.com/assets/634.png" alt="candy item"/>
    <img className="candy-item" id="36" style={{left:"56vw",top:"30vh"}} src="https://cdn.shopify.com/s/files/1/0511/5969/5552/files/bliss-home-header-cone_2400x.png?v=1614366445" alt="candy item"/>
    <img className="candy-item" id="37" style={{left:"61vw",top:"30vh"}} src="https://www.haagen-dazs.ca/sites/default/files/hdz-prod-bars_classics_va_mini.png" alt="candy item"/>
    <img className="candy-item" id="38" style={{left:"67vw",top:"30vh"}} src="https://www.mcdonalds.co.za/media/products/cone/Cone.png" alt="candy item"/>
    <img className="candy-item" id="39" style={{left:"72vw",top:"30vh"}} src="https://www.oreoicecream.com/imageGen.ashx?image=/images/products/GBR/20042012245266Oreo110mlConoclassic31011093front.png&width=600&Height=600&constrain=true" alt="candy item"/>    
    
    <label className="candy-label" style={{left:"28vw",top:"40vh"}}>31</label> 
    <label className="candy-label" style={{left:"35vw",top:"40vh"}}>32</label> 
    <label className="candy-label" style={{left:"41vw",top:"40vh"}}>33</label> 
    <label className="candy-label" style={{left:"46vw",top:"40vh"}}>34</label> 
    <label className="candy-label" style={{left:"51vw",top:"40vh"}}>35</label> 
    <label className="candy-label" style={{left:"57vw",top:"40vh"}}>36</label> 
    <label className="candy-label" style={{left:"62vw",top:"40vh"}}>37</label> 
    <label className="candy-label" style={{left:"68vw",top:"40vh"}}>38</label> 
    <label className="candy-label" style={{left:"73vw",top:"40vh"}}>39</label>
    {/* fourth row */}
    <img className="candy-item" id="41" style={{left:"27vw",top:"45vh"}} src="http://static1.squarespace.com/static/5ca3b2217980b3338106d9bf/5e9d799f49f74c43e7d4d9e2/5e8c709317a9377ba007b6af/1592901778463/Coca+Cola+300ml+Can.png?format=1500w" alt="candy item"/>
    <img className="candy-item" id="42" style={{left:"34vw",top:"45vh"}} src="https://www.mobicool.com/ui/productimages/98/13/dometic_cool-can-10_9600003942_69813_11.png?mw=580&mh=580&hash=274C18E840DFD8E457D2387BA6F840B0" alt="candy item"/>
    <img className="candy-item" id="43" style={{left:"40vw",top:"45vh"}} src="https://terroirs.co/wp-content/uploads/2016/09/p_7_7_5_775-Sprite-can-33cL.jpg" alt="candy item"/>
    <img className="candy-item" id="44" style={{left:"45vw",top:"45vh"}} src="https://www.jerseymikes.com/media/static/bedrock/lg/bottled-drink.png" alt="candy item"/>    
    <img className="candy-item" id="45" style={{left:"50vw",top:"45vh"}} src="https://pizzahousealmere.nl/wp-content/uploads/2020/02/Fanta-orange-canada.png" alt="candy item"/>
    <img className="candy-item" id="46" style={{left:"56vw",top:"45vh"}} src="https://www.tingstad.com/fixed/images/Main/1550834217/36195.png" alt="candy item"/>
    <img className="candy-item" id="47" style={{left:"61vw",top:"45vh"}} src="https://www.laudinella.ch/delivery-shop/wp-content/uploads/2014/12/RedBull.png" alt="candy item"/>
    <img className="candy-item" id="48" style={{left:"67vw",top:"45vh"}} src="https://repracores.pt/wp-content/uploads/2019/10/pepsi-cola.png" alt="candy item"/>
    <img className="candy-item" id="49" style={{left:"72vw",top:"45vh"}} src="https://i.imgur.com/MOHkCtP.png" alt="candy item"/>    
    
    <label className="candy-label" style={{left:"28vw",top:"55vh"}}>41</label> 
    <label className="candy-label" style={{left:"35vw",top:"55vh"}}>42</label> 
    <label className="candy-label" style={{left:"41vw",top:"55vh"}}>43</label> 
    <label className="candy-label" style={{left:"46vw",top:"55vh"}}>44</label> 
    <label className="candy-label" style={{left:"51vw",top:"55vh"}}>45</label> 
    <label className="candy-label" style={{left:"56vw",top:"55vh"}}>46</label> 
    <label className="candy-label" style={{left:"62vw",top:"55vh"}}>47</label> 
    <label className="candy-label" style={{left:"68vw",top:"55vh"}}>48</label> 
    <label className="candy-label" style={{left:"73vw",top:"55vh"}}>49</label>
    {/* fifth row */}
    <img className="candy-item" id="51" style={{left:"27vw",top:"60vh"}} src="https://cdn.shopify.com/s/files/1/0061/0782/4216/products/C5C52833-8F38-4E9A-AB72-F344A09F66E6_600x.png?v=1613438867" alt="candy item"/>
    <img className="candy-item" id="52" style={{left:"34vw",top:"60vh"}} src="https://www.thecandyjarby1892.com/wp-content/uploads/2020/03/chupete.png" alt="candy item"/>
    <img className="candy-item" id="53" style={{left:"40vw",top:"60vh"}} src="https://i.imgur.com/4OySEcp.png" alt="candy item"/>
    <img className="candy-item" id="54" style={{left:"46vw",top:"60vh"}} src="https://upload.wikimedia.org/wikipedia/commons/9/90/Candy-Cane-Classic_thumbnail.png" alt="candy item"/>    
    <img className="candy-item" id="55" style={{left:"50vw",top:"60vh"}} src="https://www.yogurt-land.com/assets/634.png" alt="candy item"/>
    <img className="candy-item" id="56" style={{left:"56vw",top:"60vh"}} src="https://cdn.shopify.com/s/files/1/0511/5969/5552/files/bliss-home-header-cone_2400x.png?v=1614366445" alt="candy item"/>
    <img className="candy-item" id="57" style={{left:"61vw",top:"60vh"}} src="https://www.haagen-dazs.ca/sites/default/files/hdz-prod-bars_classics_va_mini.png" alt="candy item"/>
    <img className="candy-item" id="58" style={{left:"67vw",top:"60vh"}} src="https://www.mcdonalds.co.za/media/products/cone/Cone.png" alt="candy item"/>
    <img className="candy-item" id="59" style={{left:"72vw",top:"60vh"}} src="https://www.oreoicecream.com/imageGen.ashx?image=/images/products/GBR/20042012245266Oreo110mlConoclassic31011093front.png&width=600&Height=600&constrain=true" alt="candy item"/>    
    
    <label className="candy-label" style={{left:"28vw",top:"70vh"}}>51</label>
    <label className="candy-label" style={{left:"35vw",top:"70vh"}}>52</label>
    <label className="candy-label" style={{left:"41vw",top:"70vh"}}>53</label>
    <label className="candy-label" style={{left:"46vw",top:"70vh"}}>54</label>
    <label className="candy-label" style={{left:"51vw",top:"70vh"}}>55</label>
    <label className="candy-label" style={{left:"57vw",top:"70vh"}}>56</label>
    <label className="candy-label" style={{left:"62vw",top:"70vh"}}>57</label>
    <label className="candy-label" style={{left:"68vw",top:"70vh"}}>58</label>
    <label className="candy-label" style={{left:"73vw",top:"70vh"}}>59</label>

    <label className="candy-label" style={{left:"28vw",top:"77vh"}}>5$</label> 
    <label className="candy-label" style={{left:"34vw",top:"77vh"}}>4.2$</label> 
    <label className="candy-label" style={{left:"40vw",top:"77vh"}}>1.3$</label> 
    <label className="candy-label" style={{left:"45vw",top:"77vh"}}>2.5$</label> 
    <label className="candy-label" style={{left:"50vw",top:"77vh"}}>25$</label> 
    <label className="candy-label" style={{left:"56vw",top:"77vh"}}>2.7$</label> 
    <label className="candy-label" style={{left:"61vw",top:"77vh"}}>3.4$</label> 
    <label className="candy-label" style={{left:"68vw",top:"77vh"}}>2$</label> 
    <label className="candy-label" style={{left:"73vw",top:"77vh"}}>1$</label>
    </>
  );
}}

export default Candy;
